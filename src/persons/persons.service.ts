...
export class PersonsService {
    ...
    async register(dto: CreatePersonDto){
        ...
        try{
            ...
            const post: Post = await this.postsService.getByValue('EMPLOYEE', transaction);
    
            await person.$set('posts', [post.id], {transaction});
            person.posts = [post];
            await this.historyService.create(new CreateHistoryDto(Actions.POST,
                person.id,
                tableName,
                'Пользователь зарегистрировался'), transaction);
    ...
    async login(dto: LoginPersonDto){
        ...
        try{
            ...
            await this.historyService.create(new CreateHistoryDto(Actions.POST,
                person.id,
                tableName,
                'Пользователь авторизовался'), transaction);
            await transaction.commit();
            return person;
        }catch (e){
            await transaction.rollback();
